...
Public Class FrmEquipamiento
    Private Sub FrmEquipamiento_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            Else
                ...
                For ii = 0 To FrmCompletarOT.dgEquipamiento.Rows.Count - 2
                    R = "if exists(select CustomerId, x1.EquipId, x3.NumCot from " & servidor & "[SetupCustomerEquipmentMapping] x1 inner join [MetasCotizador].[dbo].[Cotizaciones] x2
            on x1.CustomerId=x2.idContacto inner join [DetalleCotizaciones] x3 on x2.NumCot=x3.NumCot where x1.CustomerId=" & Val(FrmCompletarOT.dgEquipamiento.Item(1, ii).Value) & " and x1.EquipId=" & Val(FrmCompletarOT.dgEquipamiento.Item(2, ii).Value) & "and x3.NumCot=" & numcot & ") 
            ...
            (isnull(DetalleCotizaciones.Observaciones,' ') + isnull('OBSERVACIONES DEL SERVICIO: '+ DetalleCotizaciones.ObservacionesServicios,' ')) as Short from Cotizaciones inner join DetalleCotizaciones
            on Cotizaciones.NumCot=DetalleCotizaciones.NumCot inner join " & servidor & "[SetupEquipment] x1 on DetalleCotizaciones.EquipId=x1.EquipId  where Cotizaciones.NumCot=" & numcot & "and x1.EquipId=" & FrmCompletarOT.dgEquipamiento.Item(2, ii).Value & "end"
        ...
        Catch ex As Exception
            ...
            Bitacora("FrmEquipamiento", "Error al cargar el formmulario", Err.Number, cadena)
        End Try
    End Sub
